...
vi.mock("@/apps/Finances/expenses/components/MonthNavigation", () => ({
	MonthNavigation: () => (
		<div data-testid="month-navigation">Month Navigation</div>
	),
}));

vi.mock("@/components/AnimatedToggle", () => ({
...
vi.mock("@/components/charts", () => ({
	PieChart: ({ data }: any) => (
		<div data-testid="pie-chart">{data?.length || 0} categories</div>
	),
}));
...
describe("ExpenseOverview", () => {
	...
	it("should show placeholder data when no expenses exist", () => {
		...
		expect(
			screen.getByText(/Add expenses to see your data/i),
		).toBeInTheDocument();
	});
