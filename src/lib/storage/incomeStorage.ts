...
export class IncomeStorage {
	...
	private normalizeIncome(income: AppData["income"]): any {
		return {
			...
			weeklyTargets: [...income.weeklyTargets].sort((a, b) => a.id.localeCompare(b.id)),
	...
	async loadIncome(): Promise<AppData["income"]> {
		return this.context.queueOperation(async () => {
			...
			const targets = await this.context.db.select<Array<{ id: string; amount: number }>>(
				"SELECT * FROM income_weekly_targets"
			);

			const viewTypeResult = await this.context.db.select<Array<{ value: string }>>(
			...
			const incomeData = {
				...
				viewType: viewTypeResult.length > 0 ? (viewTypeResult[0].value as any) : "weekly",
			};
	...
	async saveIncome(income: AppData["income"]): Promise<void> {
		...
		return this.context.queueOperation(async () => {
			...
			const oldTargetIds = new Set(oldIncome?.weeklyTargets.map((t) => t.id) || []);
			...
			const deletedEntries = oldIncome?.entries.filter((e) => !newEntryIds.has(e.id)) || [];
			...
			const addedTargets = income.weeklyTargets.filter((t) => !oldTargetIds.has(t.id));
			...
			const viewTypeChanged = oldIncome && oldIncome.viewType !== income.viewType;
			...
			for (const entry of income.entries) {
				...
				await this.context.db.execute(
					...
					VALUES (?, ?, ?, ?, ?)`,
					[entry.id, entry.date, entry.amount, entry.hours || null, entry.minutes || null]
				);
			}
